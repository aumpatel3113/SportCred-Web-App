...
public class Neo4JDB {
	...
	public void storeQuestion(HttpExchange r, String question, String answer1, String answer2, String answer3,
			String answer4, String correct) {
		try (Session session = driver.session()) {
			String line = "MERGE (m:trivia {question:$a, answer1:$b, answer2:$c,answer3:$d,answer4:$e,correct:$f})";
			session.writeTransaction(tx -> tx.run(line,
					parameters("a", question, "b", answer1, "c", answer2, "d", answer3, "e", answer4, "f", correct)));
			session.close();
		} catch (Exception e) {
	...
	public int updateScore(String username, double ACS, double categoryScore, String category) {
		try (Session session = driver.session()) {
			String line = String.format("MATCH (u:User {username:$a}) SET u.ACS=$b SET u.%s=$c", category);
			session.writeTransaction(tx -> tx.run(line, parameters("a", username, "b", ACS, "c", categoryScore)));
			session.close();
			return 200;
		} catch (Exception e) {
			return 500;
		...
		try (Session session = driver.session()) {
			...
			try (Transaction tx = session.beginTransaction()) {
				...
				tx.run("CREATE (n:User {username: $u, Q1: $a, Q2: $b, Q3: $c, Q4: $d, Q5: $e, password: $v, biography: $w, "
						+ "picture: $z, ACS: $q, trivia: $x, debate: $x, picks: $x, " + "history: $x, email: $y})",
						parameters("a", Q1, "b", Q2, "c", Q3, "d", Q4, "e", Q5, "u", username, "v", password, "w",
								emptyString, "x", zeroScore, "y", email, "z", coolPicture, "q", baseScore));
	...
	public String getProfilePicture(HttpExchange r, String username) {
		try (Session session = driver.session()) {
			try (Transaction tx = session.beginTransaction()) {
				Result result = tx.run("MATCH (n{username:$x}) RETURN n.picture", parameters("x", username));
	...
	public void storeRoom(HttpExchange r, String username, JSONArray questions, String questionsAnswered,
			String questionsCorrect) {
		try (Session session = driver.session()) {
			try (Transaction tx = session.beginTransaction()) {
				String line = "CREATE (r:room {username:$b, questionsAnswered:$c,questionsCorrect:$d}) Return ID(r)";
				Result result = tx.run(line, parameters("b", username, "c", questionsAnswered, "d", questionsCorrect));

				int num = result.next().get(0).asInt();

				for (int i = 0; i < 10; i++) {
					line = "MATCH (r:room) WHERE ID(r)=$a MATCH (u:trivia) WHERE u.question=$g CREATE (r)-[z:asked]->(u)";
	...
	public void updateDebateRoom(HttpExchange r, String username, String post, int roomID, int userNum) {
		...
		try (Session session = driver.session()) {
			String line = String.format(
					"MATCH (d:debateRoom)\n MATCH(u:User {username:$b})"
							+ "WHERE ID(d)=$a\n SET d.user%s=$b\n  SET d.user%sPost=$c\n CREATE (u)-[:debated]->(d)",
					userNum, userNum);
			session.writeTransaction(tx -> tx.run(line, parameters("a", roomID, "b", username, "c", post)));
			session.close();
		} catch (Exception e) {
			internalErrorCatch(r);
		}
	...
	public void createNewDebateRoom(HttpExchange r, String question) {
		try (Session session = driver.session()) {
			...
			session.writeTransaction(tx -> tx.run(line, parameters("a", question, "b", "NULL", "c", (float) 0.0)));
			session.close();
		} catch (Exception e) {
			internalErrorCatch(r);
		}
	...
	public int[] getDebateRoom(HttpExchange r, String question) {
		try (Session session = driver.session()) {
			try (Transaction tx = session.beginTransaction()) {
				String line = "MATCH (d:debateRoom {question:$a, user3:$b})\n WHERE NOT d.user2=$b\n Return ID(d)";
...
//        return null;
	...
	public Map<String, Object> isValidPost(HttpExchange r, String username) {
		try (Session session = driver.session()) {
			try (Transaction tx = session.beginTransaction()) {
				boolean valid = true;
				String line = "MATCH(u:User {username:$a}) MATCH(d:debateRoom) MATCH ((u)-[f:debated]->(d)) RETURN(f)";
				Result result = tx.run(line, parameters("a", username));
	...
	public Map<String, Object> getDebateGroup(HttpExchange r, String username) {
		try (Session session = driver.session()) {
			try (Transaction tx = session.beginTransaction()) {
				String line = "MATCH(d:debateRoom) WHERE NOT d.user3=$b MATCH(u:User {username:$a}) WHERE NOT "
				...
				if (result.hasNext()) {
					...
					Object[] scores = { temp.get("user1Score"), temp.get("user2Score"), temp.get("user3Score") };
	...
	public int addZoneComment(String currentUsername, int parentPostID, String commentContent) {
		...
		try (Session session = driver.session()) {
			...
			try (Transaction tx = session.beginTransaction()) {
				...
				Result checkCommentNumber = tx.run(
						"MATCH (p:ZonePost {postID: $x}), (c)-[r:COMMENT_ON]->(p) RETURN COUNT(c)",
						parameters("x", parentPostID));
	...
	public List<JSONObject> getZoneFeed(String currentUser) {
		...
		try (Session session = driver.session()) {
			...
			try (Transaction tx = session.beginTransaction()) {
				...
				Result allPost = tx
						.run("MATCH (p:ZonePost) RETURN p.postID, p.author, p.content, p.likeCount, p.dislikeCount");
				Result allComments;
				Result checkRatedByUser;
				Result currentACSScore;
...
//					currentACSScore = tx.run("MATCH (n:User {username: $a}) RETURN n.ACS",
//							parameters("a", currentPostAuthor));
//					
//					String currentPostAuthorACS = currentACSScore.next().get(0).toString();
					
					UserNode currentPostAuthorACS = new UserNode(currentPostAuthor);
					int currentACS = (int)currentPostAuthorACS.getACS();
					...
					while (allComments.hasNext()) {
						...
						UserNode commentAuthor = new UserNode((String) currentComment.asMap().get("c.author"));
						String commentAuthorACS = ""+ (int) commentAuthor.getACS();
						
						String completeComment = author + "~" + commentAuthorACS + ": " + commentContent;
					...
					postAsJSON.put("authorACS", currentACS);
	...
	public void storePlayoffBracket(HttpExchange r, String playoffsID, PlayoffSeries[] series) {
		try (Session session = driver.session()) {
			String pre = String.format("MERGE (n:playoffBracket {playoffsID:'%s',numSeries:%s", playoffsID,
					series.length);
				...
				for (Record x : records) {
					...
					for (int i = 0; i < numSeries; i++) {
						...
						response += "\t\t\t\t{\"id\" : \"" + current.get(0) + "\", \"round\" : \"" + current.get(1)
								...
								+ "\"," + " \"winner\" : \"" + current.get(4) + "\", \"seriesLength\" : \""
								+ current.get(5) + "\"},\n";
					}
	...
	public String storeBracketPrediction(HttpExchange r, String username, String playoffsID, PlayoffSeries[] series) {
		try (Session session = driver.session()) {
			Boolean exists = checkForUserBracket(username, playoffsID);

			if (!exists) {
				String pre = String.format(
						"MERGE (n:playoffBracketPrediction {username:'%s'," + "playoffsID:'%s',numSeries:%s", username,
		...
		try (Session session = driver.session()) {
			try (Transaction tx = session.beginTransaction()) {
				String query = String.format(
						"MATCH(n:playoffBracketPrediction {username:'%s', playoffsID:'%s'})" + "RETURN n", username,
						playoffsID);
	...
	private String compareBracketPrediction(HttpExchange r, PlayoffSeries[] prediction, String playoffsID,
			String username, Boolean exists) {
		try (Session session = driver.session()) {
			try (Transaction tx = session.beginTransaction()) {
				String response = "{\n\t\"username\" : \"" + username + "\",\n\"playoffsID\" : \"" + playoffsID
						+ "\",\n";
				...
				for (int i = 0; i < prediction.length; i++) {
					...
					response += String.format(
							...
							i + 1, prediction[i].getWinner(), correct);
				}
				...
				if (result.hasNext()) { // prediction data exists
					...
					for (int i = 0; i < numSeries; i++) {
						...
						response += String.format("\t\"series%d\" : {\"id\" : \"%s\", \"team\" : \"%s\"},\n", i + 1,
								records.get(0).get(0).asString(), records.get(0).get(1).asString());
					}
					response = response.substring(0, response.length() - 2) + "\n}";
					return response;
				}
	...
	private void updateDebateValues(HttpExchange r, String ratingUser, double newScore, double voters, String position,
			String scoredUser) {
		try (Session session = driver.session()) {
			String line = String
					.format("MATCH(u:User {username:$a})\n MATCH (d:debateRoom {%s:$b})\n SET d.%sScore=$c\n "
							+ "SET d.%sVotes=$e\n " + "CREATE (u)-[:voted]->(d)", position, position, position);
			session.writeTransaction(
					tx -> tx.run(line, parameters("a", ratingUser, "b", scoredUser, "c", newScore, "e", voters)));
			session.close();
		} catch (Exception e) {
			e.printStackTrace();
			internalErrorCatch(r);
