...
import './OnlinePostTrivia.css';
import axios from 'axios'

const OnlinePostTrivia = () => {
    ...
    useEffect(() => {
        if (sendResults === 1) {
            const url = 'http://localhost:8080/api/v1/sendNewChallengerTrivia'
            const headers = {
                'Content-Type': 'text/plain',
            }

            const justQuestions = [questions[0].question, questions[1].question, 
            questions[2].question, questions[3].question, 
            questions[4].question, questions[5].question,
...
export default OnlinePostTrivia;